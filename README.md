...
**（１）ダイナミクセルのシャフトが，±４５度の範囲で，周波数２Ｈｚで正弦波で振動するようにプログラムを改良して実行してみてください．**
...
> [!TIP]
> 完成したプログラムファイルは，名前を変えたコピーを保存するなりして残しておくようにしましょう．もし，今後修正してコンパイルエラーが出た時にも，昔のファイルがあればデバッグが簡単になります．

**（２）ダイナミクセルのシャフトが，0-90度の範囲で，45度を中心として，周波数 1Ｈｚで正弦波で振動するようにプログラムを改良して実行してみてください．**

> [!NOTE]
> プログラム上で，振幅 amp と角速度 omega の値を適切に変えてみましょう．
> さらに，中心の角度を変えるために，オフセット offset を定義する必要がありますね．
> 実数変数 offset の定義は，
> ```c
> double offset = 0; 
> ```
> と行を追加すれば定義できることをヒントに，プログラムを修正してください．
> プログラムを修正したあとは，ファイルを保存して，コンパイルして，実行し直すのを忘れないでください．
...
**（３）周波数をスイープさせてみましょう．周波数以外は(2)と同じ条件にして，周波数を 0-2Hzまで，時間の一次関数で増やしていってください．200sくらいで2Hzになるようにしましょう**

> [!NOTE]
> スイープというのは，振動周波数を徐々に変えていくことになります．
> プログラムを修正したあとは，ファイルを保存して，コンパイルして，実行し直すのを忘れないでください．
> デバッグのために，現在の周波数を表示するようにするとわかりやすいですね．

> [!TIP]
> 例えば，表示部分に以下のように４行目を追加してやれば，周波数が表示できそうですね．
> ```c++
> if (count % 100 == 1) {
> 	std::cout << " Pos : " << s_angle << ", Spd : " << s_omega << std::endl;
> 	std::cout << " Load: " << s_load << ", Volt : " << s_volt << ", Tmp : " << s_temp << std::endl;
>  	std::cout << " Present Oscillation Freq: " << omega / (2 * am::pi) << " Hz " << std::endl;
> }
> ```
